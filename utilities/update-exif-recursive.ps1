# Name: GoInfoGame Images Exif Data Update Script (Recursive Version)
# Version: 1.1
# Description: This script removes all metadata from PNG files in a specified directory and its subdirectories and adds CC0 copyright tags to each image.
...
Write-Host "GoInfoGame Images Exif Data Update Script (Recursive) v1.1" -ForegroundColor Blue
...
# Get all PNG files in the specified directory and its subdirectories
$pngFiles = Get-ChildItem -Path $directory -Filter *.png -File -Recurse

# Exit if no PNG files are found
if ($pngFiles.Count -eq 0) {
    Write-Host "No PNG files found in the specified directory or its subdirectories." -ForegroundColor Yellow
    exit 0
}
...
Write-Host "Exif data updated for all PNG files in the directory and its subdirectories." -ForegroundColor DarkBlue
